...
SAMPLER2D(s_texDiff,  2);
SAMPLER2D(s_texNormal, 3);
SAMPLER2D(s_texSpecular, 4);
...
void main()
{
	...
	// calculate sample normal from normal map
	vec3 T = normalize(v_tangent);
	vec3 B = normalize(v_bitangent);
	vec3 N = normalize(v_normal);
	mat3 TBN = mtx3FromCols(T, B, N);
	...
	vec3 inAlbedo = texture2D(s_texDiff, v_texcoord0).rgb;

	// float inReflectivity = u_reflectivity;
	float inReflectivity = texture2D(s_texSpecular, v_texcoord0).r;
	...
	vec3 refl = u_rgbSpec.xyz * vec3_splat(inReflectivity);
	...
	gl_FragColor.w = texture2D(s_texNormal, v_texcoord0).b;
}
